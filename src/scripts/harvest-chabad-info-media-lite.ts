// src/scripts/harvest-chabad-info-media-lite.ts
...
const DEFAULT_MEDIA_CATEGORIES = [
  "https://chabad.info/media_category/chabad/",
  "https://chabad.info/media_category/limed/",
  "https://chabad.info/media_category/nneimotchabad/",
];
...
const MAX_PAGES_PER_CAT = Number(process.env.CI_MEDIA_PAGES || 30);
const DELAY_MS = Number(process.env.CI_DELAY || 250);
const UA = { "User-Agent": "maty-music/harvest-ci-media-lite (+)" };
...
async function listPostLinks(listUrl: string): Promise<string[]> {
  ...
  // לוקחים רק כותרות פוסטים כדי לצמצם רעש
  $(".entry-title a, article .entry-title a").each((_, a) => {
    const href = $(a).attr("href");
    if (href && href.startsWith("http")) links.add(href.split("#")[0]);
  });

  // fallback אם אין entry-title
  if (links.size === 0) {
    $("article a").each((_, a) => {
      const href = $(a).attr("href");
      if (href && href.startsWith("http")) links.add(href.split("#")[0]);
...
function extractFromPost(html: string) {
  ...
  $('a[href$=".mp3"], audio source[src$=".mp3"], audio[src$=".mp3"]').each(
    (_, el) => {
  ...
  // @ts-ignore
  $.root && $.root();
  return { title, cover: normalizeUrl(cover), tags: [...new Set(tags)], mp3s: [...mp3s] };
}
...
main().catch((e) => {
  console.error("[FATAL] harvest-chabad-info-media-lite:", e);
  process.exit(1);
});
