...
export function getTransport(): Transporter {
  ...
  // האם לאפשר self-signed (לבדיקות / פרוקסי ארגוני)
  const allowSelfSigned =
    envOn("SMTP_ALLOW_SELF_SIGNED") ||
    envOn("SMTP_TLS_INSECURE") ||
    envOn("ALLOW_SELF_SIGNED_CERTS");

  if (!host || !user || !pass) {
    // מצב דמו: לא שולח באמת, רק מדפיס ל-console
  ...
  _cached = nodemailer.createTransport({
    host,
    port,
    secure: port === 465, // 465 = TLS ישיר; 587 = STARTTLS
    auth: { user, pass },
    pool: true,
    maxConnections: 3,
    maxMessages: 100,
    ...
    tls: allowSelfSigned
      ? {
          // מאפשר self-signed – להשתמש בזה רק אם אתה באמת צריך
  ...
  <div style="direction:rtl;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#111;background:#fff;padding:24px;line-height:1.55">
    <div style="max-width:640px;margin:auto">
      ...
      <div style="font-size:12px;opacity:.7;margin-top:16px">
        אימייל נשלח אוטומטית מהמערכת. מענה: ${reply}${tel ? " · טל׳ " + tel : ""}
      </div>
    </div>
  </div>`;
}
