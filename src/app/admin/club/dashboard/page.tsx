...
/**
 * AdminExecutiveDashboard — MATY-CLUB Edition
 * -----------------------------------------------------------
 * KPIs, תרשימי SVG (דונאט/ספרקליין), פילטרים, חיפוש/מיון/עימוד,
 * ייצוא CSV, הדפסה, רענון אוטומטי, קיצורי־מקלדת, RTL מלא.
 * עובד מול /api/admin/metrics?scope=club (ראה הראוט שסיפקתי).
 * -----------------------------------------------------------
 */
...
type MetricPayload = {
  totals: {
    profiles: number; // סה״כ משתמשים (users)
    new7d: number; // מצטרפים 7 ימים
    updated24h: number; // פוסטים/תכנים חדשים 24ש׳
    online10m: number; // מחוברים ≈10 דקות אחרונות
    avgAge: number | null; // ב-CLUB לא בשימוש (נשאר לתאימות)
  };
  // ב-CLUB: byGender=byType (video|audio|text), byDirection=byGenre, byGoal=byTag
...
async function fetchMetrics(signal?: AbortSignal) {
  const r = await fetch("/api/admin/metrics?scope=club", {
    cache: "no-store",
...
function Sparkline({
  ...
  const area =
    points.length > 1
      ? `${path} L ${points[points.length - 1][0]},${height - 2} L ${points[0][0]},${height - 2} Z`
      : "";
...
function StatGroup({
  ...
  // ב-CLUB נציג KPI אחרון כ"עדכונים היום" (לא גיל)
  const todayCount = dailySeries.length
    ? dailySeries[dailySeries.length - 1].count
    : 0;
  const sumWindow = dailySeries.reduce((a, b) => a + b.count, 0);
  const change =
    prevWindowSum != null ? percentChange(sumWindow, prevWindowSum) : null;
  ...
  return (
    <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <KPI
        icon="shield"
        label="מצטרפים 7 ימים"
        value={loading ? "…" : fmt.format(totals?.new7d ?? 0)}
      />
      <KPI
        icon="gear"
        label="פוסטים חדשים · 24ש׳"
      ...
      <KPI
        icon="user"
        label="עדכונים היום"
        value={loading ? "…" : fmt.format(todayCount)}
        change={change}
        sub={
          <div className="mt-2">
...
function DistributionCards({
  ...
  const byTypeDonut = (byGender || []).map((x) => ({
    key: x.key,
    value: x.count,
  })); // video/audio/text
  const byGenreDonut = (byDirection || []).map((x) => ({
    key: x.key,
    value: x.count,
  })); // ז'אנרים
  const byTagDonut = (byGoal || []).map((x) => ({
    key: x.key,
    value: x.count,
  })); // תגיות
  ...
  return (
    <div className="mt-6 grid gap-6 xl:grid-cols-3">
      <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
        <div className="mb-3 font-bold">פילוח לפי סוג תוכן</div>
        {loading ? (
          <Skeleton className="h-40" />
        ) : (
          <DonutChart data={byTypeDonut} />
        )}
      </div>
      <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
        <div className="mb-3 font-bold">ז׳אנרים</div>
        {loading ? (
          <Skeleton className="h-40" />
        ) : (
          <DonutChart data={byGenreDonut} />
        )}
      </div>
      <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
        <div className="mb-3 font-bold">תגיות מובילות</div>
        {loading ? (
          <Skeleton className="h-40" />
        ) : (
          <DonutChart data={byTagDonut} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="mt-6 rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4 overflow-x-auto">
      <div className="mb-3 flex items-center gap-2 font-bold">
        <span>עדכונים אחרונים (פרופילים)</span>
...
export default function AdminExecutiveDashboard() {
  ...
  const [range, setRange] = useLocalStorage<7 | 14 | 30 | 90>(
    "admin:range",
    30,
  );
  ...
  async function load() {
    ...
    try {
      ...
      setHistoryOnline((h) => [...h, j.data.totals.online10m].slice(-20));
    } catch (e: any) {
  ...
  React.useEffect(() => {
    load();
    function vis() {
      if (document.hidden) controllerRef.current?.abort();
    }
  ...
  React.useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (!mod && e.key === "/") {
        e.preventDefault();
        document
          .querySelector<HTMLInputElement>("input[placeholder*='חיפוש']")
          ?.focus();
      }
  ...
  function handleExportCSV() {
    ...
    const csv =
      ...
      rows
        ...
        .join("\n");
    downloadBlob(`maty-club-recent-${Date.now()}.csv`, csv);
  }
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 print:px-0" dir="rtl">
      <header className="mb-3">
        <h1 className="text-3xl font-extrabold">פאנל מנהלים — MATY-CLUB</h1>
        <p className="opacity-70">סקירה חכמה ל־CLUB (עדכונים חיים)</p>
      </header>
      ...
      {error && (
        <div className="mb-4 rounded-xl border border-rose-300/40 bg-rose-100/80 dark:bg-rose-500/10 p-3 text-rose-800 dark:text-rose-300">
          {error} — ודא הרשאות ADMIN ושה־API זמין.
        </div>
      )}
      ...
      {/* Quick insights (CLUB semantics) */}
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
          <div className="flex items-center gap-2 text-sm opacity-80">
            <Icon name="user" /> סוג תוכן מוביל
          </div>
        ...
        <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
          <div className="flex items-center gap-2 text-sm opacity-80">
            <Icon name="target" /> תגית מובילה
          </div>
          <div className="mt-1 text-lg font-bold">{loading ? "…" : topTag}</div>
          <div className="mt-2">
      ...
      <div className="mt-6 rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/70 p-4">
        <div className="mb-3 flex items-center gap-2 font-bold">
          <span>מחוברים בזמן אמת (20 דגימות אחרונות)</span>
          <Badge tone="muted">
